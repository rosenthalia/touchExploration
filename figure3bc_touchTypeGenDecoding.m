...
if toLoad == 1
    ...
    load(['..\Data\40dimSVD_LDAdecoding_touchTypeGen_0.5sbins.mat']);
...
f2 = plotConfMats(acc_half(probsATrain,:,:),nullacc_half(probsATrain,:,:),...
    ...
    8:12, probs_byTy_g(probsATrain,1),probs_byTy_g(probsATrain,2));

% plot train on finger accuracies
probTitle = ['train F test A'];
f3 = plotConfMats(acc_half(probsFTrain,:,:),nullacc_half(probsFTrain,:,:),...
    conds_ty, [sgt ' - ' probTitle], numBins, binWidth,...
    8:12, probs_byTy_g(probsFTrain,1), probs_byTy_g(probsFTrain,2));
...
function fig = plotConfMats(acc_half_confMat, null_acc_confMat,conds,sgt,...
    numBins, binWidth, binsToPlot, ty1, ty2)
numCond = numel(conds);
%trainID  1 if train on arm, 2 if train on finger
%testID 1 if train on arm, 2 if train on finger
mean_accs= squeeze(mean(acc_half_confMat, 2)); % %mean across reps, becomes bin x nsp x prob acc
postStimBinLen = 40;
preStimBinLen = 80;
...
for bn = binsToPlot
    for pb = 1:size(ty1,1)
        cr = ty1(pb); % row is type you train on
        cc = ty2(pb); % column is type you test on
        % transform into heatmap
        meanAccMap(bn,cr,cc) = mean_accs(pb,bn);
        meanAccMap(bn,cc,cr) = mean_accs(pb,bn); %symmetric
        ...
        % check is null is greater than acc
        if nullacc_CI05(bn, cr,cc,1)>=acc_CI05(bn, cr,cc, 2) % is real acc sig. greater than null acc?
...
for bn = binsToPlot
    ...
    xlabel('class 2')
    xtickangle(-40)
    title(['S1: ' num2str(Lbs(bn)) ' - ' num2str(Lbs(bn+1)) 's'], 'Interpreter', 'none')
    caxis([chance 1])
    colormap(cmap);
        ...
        for cc = 1:numCond
            if size(CIsig_eff{bn,cr,cc},2)>0 % if significant
                if size(CIsig_eff{bn,cr,cc},2)<3 % just to make it pretty, format *** differently
                    ...
                    text(cc,cr+0.2,sprintf('%s','*'), 'FontSize', 10, 'HorizontalAlignment', 'center');
                end
            end
        end
    end
end
sgtitle([sgt ' accuracy heatmaps'], 'Interpreter', 'none')
end