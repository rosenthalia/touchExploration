...
% then plots the decoding results as shown in Figure 3(a)
...
if toLoad == 1
    ...
    load(['..\Data\40dimSVD_LDAdecoding_effGen_0.5sbins.mat']);
...
f2 = plotConfMats(acc_half,nullacc_half, conds_ty, sgt, numBins, binWidth,...
    8:12, trainType, testType);

% get numbers for paper
% accs and CIs in 0.5
disp(['0-0.5t: a v f train *BL test *FP: ' num2str(mean(acc_half(7,:,9))) ' ['...
    num2str(prctile(acc_half(7,:,9),2.5)) '-'...
    num2str(prctile(acc_half(7,:,9),97.5)) ']'])
...
function fig = plotConfMats(acc_half_confMat, null_acc_confMat, conds, sgt,...
    numBins, binWidth, binsToPlot, trainType, testType)
...
for bn = binsToPlot
    for pb = 1:size(trainType,1)
        cr = trainType(pb); % row is type you train on
        cc = testType(pb); % column is type you test on
