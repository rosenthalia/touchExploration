function  [pearsonR, p_pearson, pats_mds_2D] = figureMDSArrangement(RDM, userOptions, localOptions)
...
if isfield(localOptions,'figI_shepardPlots')&&~isempty(localOptions.figI_shepardPlots)
    ...
    [pearsonR, p_pearson] = shepardPlot_iar(D,disparities,pdist(pats_mds_2D),localOptions.figI_shepardPlots,['\fontsize{12}MDS (' description{1} ')']);
else
    pearsonR = [];
    p_pearson = [];
end

%% rotate the MDS plot in some custom orientation
if isfield(localOptions, 'rotateAngle')
    alpha = localOptions.rotateAngle;
    unrotatedPoints = pats_mds_2D;
    
    rotX = [cos(alpha) -sin(alpha); sin(alpha) cos(alpha)]; %rotation around X axis
    pats_mds_2D = [rotX*unrotatedPoints']'; % rotate
end
...
if isfield(localOptions,'figI_textLabels')&&~isempty(localOptions.figI_textLabels)
    ...
    if ~exist('contrasts','var')
        ...
        veryLocalOptions.markerStyles = localOptions.markerStyles;
