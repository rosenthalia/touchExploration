function [pearsonR, p_pearson, pointCoords] = MDSConditions_iar(RDMs, userOptions, localOptions)
...
MDSOptions.markerStyles = userOptions.markerStyles;
...
for RDMi = 1:nRDMs
    ...
    %
    if isfield(localOptions, 'rotateAngle')
        veryLocalOptions.rotateAngle = localOptions.rotateAngle;
    end
    if isfield(localOptions,'fontSize')
        veryLocalOptions.fontSize = localOptions.fontSize;
    end
    if ~isfield(localOptions,'figI_textLabels')
        veryLocalOptions.figI_textLabels = [thisFigureNumber];
    else
        veryLocalOptions.figI_textLabels = localOptions.figI_textLabels;
    end
    
    if ~isfield(localOptions,'figI_shepardPlots')
        veryLocalOptions.figI_shepardPlots = [thisFigureNumber 2 1 1];
    else
        veryLocalOptions.figI_shepardPlots = localOptions.figI_shepardPlots;
    end
    ...
    if ~isnan(veryLocalOptions.figI_textLabels)
    ...
    if ~isempty(veryLocalOptions.figI_shepardPlots)
        [pearsonR, p_pearson, pointCoords] = figureMDSArrangement_iar(RDMs(RDMi), userOptions, veryLocalOptions);
    else
        [~,~, pointCoords] = figureMDSArrangement(RDMs(RDMi), userOptions, veryLocalOptions);
        pearsonR = [];
        p_pearson = [];
    end
end%for
